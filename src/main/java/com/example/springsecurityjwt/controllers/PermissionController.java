...
public class PermissionController {
    ...
    public ResponseEntity<?> findAllPermissions() {
        return ResponseEntity.ok(permissionService.findAllRoles());
    }

    @PostMapping("/save")
    //@PreAuthorize("hasRole('ROLE_ADMIN')")
    public ResponseEntity<?> savePermission(@Valid @RequestBody PermissionRequest permissionRequest) {
        PermissionResponse permissionResponse = permissionService.saveRole(permissionRequest);
